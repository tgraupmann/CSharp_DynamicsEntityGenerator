...
namespace DynamicsEntityGenerator
{
    public class AutoGeneratorClient
    {
        ...
        public IOrganizationService GetServiceProxy()
        {
            return _mServiceProxy;
        }
        ...
        public EntityReference NewEntityReference<T>() where T : IAutogeneratedClass
        {
            ...
            return new EntityReference(tableName);
        }
        ...
        public T Retrieve<T>(Guid id, ColumnSet columnSet) where T : IAutogeneratedClass, new()
        {
            ...
            if (record != null)
            {
                ...
                foreach (var ac in record.Attributes)
                {
                    ...
                    {
                        Console.Error.WriteLine("{0} Attribute Not Mapped! Key={1} Value={2}",
                            GetTableName<T>(), ac.Key, ac.Value);
                    }
                }
                //formatted values
                foreach (var fv in record.FormattedValues)
                {
                    ...
                    {
                        Console.Error.WriteLine("{0} Formatted Value Not Mapped! Key={1} Value={2}",
                            GetTableName<T>(), fv.Key, fv.Value);
                    }
                }
                return item;
            }
